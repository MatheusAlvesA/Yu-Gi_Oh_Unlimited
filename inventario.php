...
<html lang="pt">
  <head> <?php include 'head.txt';?>
    ...
    <title>Inventário Yu-Gi-Oh Unlimited</title>
...
<div class="row">
  ...
  <h1 style="text-align: center;" class="oficial-font">SEU INVENTARIO</h1>
  <?php echo act();?>
   <table class="table table-striped" cellspacing="0" cellpadding="0">
    <?php echo cartas($db->cards, $pagina);?>
    </table>
    <?php
      if($paginacao == 1) { // tem próxima página
        echo '<div class="row"><div class="col-md-2" style="float: right;"><a href="inventario.php?pagina=2"><button class="btn btn-primary btn-lg">Próxima</button></a></div></div>';
      } elseif($paginacao == 2) { // tem página anterior
        echo '<div class="row"><div class="col-md-2" style="float: right;"><a href="inventario.php"><button style="float: right;" class="btn btn-primary btn-lg">Anterior</button></a></div></div>';
      }
    ?>
  </div>
...
<?php
 ...
 function cartas($deck, $pagina) { // retorna a lista de cartas
  if($deck[0] <= 1) {return '<tr><td align="center" valign="middle"><b>Você ainda não possui cartas no seu inventário. Entre no <a href="comercio.php">comércio de cartas</a> para começar a formar seu deck.</b></td></tr>'."\n";}
  if(($deck[0] - 1) <= 50) {$pagina = 1;}
  if(($deck[0] - 1) == 1) {return '<tr><td><a href="javascript:abrir(\'info_card.php?fonte=2&id='.$deck[1].'\')"><img src="imgs/cards/pequenas/'.$deck[1].'.png" class="img-responsive" /></a></td></tr>'."\n";}
  if(($deck[0] - 1) == 2) {return '<tr><td><a href="javascript:abrir(\'info_card.php?fonte=2&id='.$deck[1].'\')"><img src="imgs/cards/pequenas/'.$deck[1].'.png" class="img-responsive" /></a></td><td><a href="javascript:abrir(\'info_card.php?fonte=2&id='.$deck[2].'\')"><img src="imgs/cards/pequenas/'.$deck[2].'.png" class="img-responsive" /></a></td></tr>'."\n";}
  ...
  if($pagina == 1) {
   ...
   $y = 0;
   $retorno = '<tr>';
   while($x <= ($deck[0] - 1) && $x <= 50) {
    if($y == $Nlinha) {$retorno .= '</tr>'."\n".'<tr>'; $y = 0;}
    $retorno .= '<td><a href="javascript:abrir(\'info_card.php?fonte=2&id='.$deck[$x].'\')"><img src="imgs/cards/pequenas/'.$deck[$x].'.png" class="img-responsive" /></a></td>';
    $x++;
    $y++;
   }
 $retorno .= '</tr>'."\n";
 if(($deck[0] - 1) > 50) $paginacao = 1; // tem próxima
   return $retorno;
  }
  else { // se estiver na pagina 2
  if(($deck[0] - 51) == 1) {$paginacao = 2;return '<tr><td><a href="javascript:abrir(\'info_card.php?fonte=2&id='.$deck[51].'\')"><img src="imgs/cards/pequenas/'.$deck[51].'.png" class="img-responsive" /></a></td></tr>'."\n";}
  if(($deck[0] - 51) == 2) {$paginacao = 2;return '<tr><td><a href="javascript:abrir(\'info_card.php?fonte=2&id='.$deck[51].'\')"><img src="imgs/cards/pequenas/'.$deck[51].'.png" class="img-responsive" /></a></td><td><a href="javascript:abrir(\'info_card.php?fonte=2&id='.$deck[52].'\')"><img src="imgs/cards/pequenas/'.$deck[52].'.png" class="img-responsive" /></a></td></tr>'."\n";}
    $x = 51; // a primeira pagina mostro as primeiras 25 cartas
   $y = 0;
   $retorno = '<tr>';
   while($x <= ($deck[0] - 1)) {
    if($y == $Nlinha) {$retorno .= '</tr>'."\n".'<tr>'; $y = 0;}
  ...
  $retorno .= '</tr>';
  $paginacao = 2; // tem anterior
   return $retorno;
 }
}

function act() {
if($_GET['movida'] == 1) {return "<div class=\"alert alert-success\" role=\"alert\"><strong>Sucesso!</strong> A carta foi movida para seu DECK</div>";}
if($_GET['removida'] == 1) {return "<div class=\"alert alert-danger\" role=\"alert\"><strong>Sucesso!</strong> A carta foi removida do seu Inventário</div>";}
}
