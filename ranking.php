...
if(!file_exists('ranking.txt')) {atualizar();header('location: ranking.php');exit();}
else {
    $grav = new Gravacao();
    $grav->set_caminho('ranking.txt');
    $ranking = $grav->ler(0);
    unset($grav);
...
// agora mostrar na tela corretamente
?>
<!DOCTYPE html>
<html lang="pt">
  <head> <?php include 'head.txt';?>
    ...
    <title>RANKING Yu-Gi-Oh Unlimited</title>
  ...
  <div role="main" class="col-md-6 col-md-push-3">
    ...
    <div class="table-responsive col-md-12">
      <ul class="nav nav-tabs">
        <li class="active"><a href="#">Duelistas</a></li>
        <li><a href="ranking_clans.php">Clãs</a></li>
      </ul>
...
function lista($ranking) {
    $banco = new DB();
 if($ranking[0]-2 == 0) {return '<tr><td colspan="2"><b>Nenhum duelista entrou no Ranking ainda</b></td></tr>';}
 for($x = 2;$x < $ranking[0];$x++) {
    $banco->ler($banco->nome_id($ranking[$x]));
    if($banco->xp != 0) $xp = $banco->xp;
    else $xp = '???';
   $retorno .= '<tr><td align="center"><b>'.($x-1).'°</b></td><td align="right"><a href="user.php?nome='.$ranking[$x].'"><b>'.$ranking[$x].'</b></a></td><td><b>'.$xp.'</b></td></tr>'."\n";
 }
...
function atualizar() {
    ...
    for($x = 0; $x < $tamanho; $x++)
     $texto .= $array[$x]."\n";
    file_put_contents('ranking.txt', substr($texto, 0, -1));
}
