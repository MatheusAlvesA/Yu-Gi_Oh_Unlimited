...
/* carta terminada em 07/02/2017 terminada em 1 dia
 * Durante sua MainPhase, você pode Invocar por Invocação-Normal 1 monstro (Constelar) em adição a sua Invocação Normal
 * padrão do turno. (Você só pode se aproveitar deste efeito uma vez por turno,
 * independente de quantos Constellar Leonis possua.)
 */
class c_130 extends Monstro_normal {
  ...
  if(file_exists($this->pasta.'130_leonis_ativado.txt')) {
  ...
  function carta_solicitada($cartaS) {
    if(file_exists($this->pasta.'130_leonis_ativado.txt')) return false;
   ...
   file_put_contents($this->pasta.'130_leonis_ativado.txt', $lista[$cartaS]);
   return true;
  }
...
function Comandos() {
  ...
  fclose($arq);
   ...
   if(file_exists($this->pasta.'130_leonis_ativado.txt')) $comandos_possiveis['ativar'] = false;
 ...
 function tarefa($txt) { //essa carta deve se destruir caso seja ativada e round passe
     @unlink($this->pasta.'130_leonis_ativado.txt');
     return true;
 }
