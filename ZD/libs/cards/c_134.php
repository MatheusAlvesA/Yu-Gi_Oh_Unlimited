...
/* carta terminada dia 12/02/2017. terminada em 1 dia
 * Quando essa carta é Normal Summon: Você pode adicionar 1 monstro (Constellar) do seu Deck para sua Mão.
 */
class c_134 extends Monstro_normal {
 ...
 private function _ativar() {  // Efeito desse monstro
    ...
    $mao = $this->duelo->ler_deck($this->dono);
    $y = 0;
    for($x = 1; $x < $mao[0]; $x++) {
 ...
 if(!isset($lista)) {
     parent::avisar('Não existem outros Constelares em seu deck');
     return false;
 }
  ...
  function carta_solicitada($cartaS) {
    ...
    parent::avisar('Efeito do monstro '.$this->nome.' ativado, '.$carta->nome.' foi puxada', 1);
