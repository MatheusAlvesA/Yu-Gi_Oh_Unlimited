<?php
// terminada 01/07/2016
// terminada em 1 minuto por já existir outras iguais

class c_71 extends Magica {
/* Esta carta é utilizada para o Ritual Summon de (Magician of Black Chaos).
 * Você deve Tributar monstros do seu lado do campo ou da sua mão até obter um Nível total igual a 8 ou maior.
 */
...
function carta_solicitada($cartaS) {
 ...
 function ritual() { // essa função é executada deposi dos sacrificios serem feitos
    ...
    $existe = false;
    for($x = 1; $x < $mao[0]; $x++) {
        if($mao[$x] == 454) { // id da carta Magician of Black Chaos
            parent::excluir_carta_hand('id', 454);
            $existe = true;
            break;
        }
    }
    ...
    if($x > 5) {
     parent::avisar('Não existe espaço para invocar o Magician of Black Chaos');
     $this->tarefa('x');
     return false;
    }
     parent::avisar('Carta ritual '.$this->nome.' ativada', 1);
     $this->duelo->invocar($this->dono, $x, 1, 454, 'ritual'); // invocando de forma ritual
     $this->tarefa('x');
     return true;
    }
    else {
     parent::avisar('Magician of Black Chaos não foi encontrado na sua mão');
