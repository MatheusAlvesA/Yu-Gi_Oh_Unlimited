<?php
class c_186 extends Monstro_normal {
    /* terminada dia 30/03/2017. terminada em 1 dia
     * Se esta carta ataca, ela ganha 1200 de ATK apenas durante a Damage Step.
    */
	 ...
	 if(parent::ler_variavel('atacou_em') == parent::ler_turno()) {parent::avisar('Você não pode atacar mais de uma vez com o mesmo monstro no mesmo turno'); return 0;}
	  ...
	  if(parent::checar($gatilho)) {
	   ...
	   if($resposta['bloqueado']) {
          ...
          $this->duelo->alterar_lp((-1)*($this->atk+1200), $this->duelo->oponente($this->dono)); //efeito
          parent::manter('atacou_em', parent::ler_turno()); 
	  ...
	  elseif($resposta['resultado'] == 'v') {
           if($alvo->modo == MODOS::ATAQUE) parent::dano_direto($lps, (int)$resposta['sobra']+1200);//efeito
	   parent::avisar($this->nome.' atacou '.$alvo->nome.' que foi destruído', 1);
         ...
         if($alvo === 'direto_s' && $retorno === true) $this->duelo->alterar_lp((-1200), $this->duelo->oponente($this->dono)); //efeito
         if($retorno['resultado'] === 'v') $this->duelo->alterar_lp((-1200), $this->duelo->oponente($this->dono)); //efeito
         return $retorno;
        }
       }   
