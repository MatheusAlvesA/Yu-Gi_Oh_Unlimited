<?php
class c_607 extends Magica {
/* carta terminada dia 23/04/2017. terminada em 1 dia
 * Esta carta é usada para Ritual Summon (Dokurorider).
 * Você também deve tributar monstros do seu campo ou mão cujo total de Níveis seja igual ou superior a 6.
 */
...
function carta_solicitada($cartaS) {
 ...
 function ritual() { // essa função é executada depois dos sacrificios serem feitos
    ...
    $existe = false;
    for($x = 1; $x < $mao[0]; $x++) {
        if($mao[$x] == 206) { // id da carta Dokurorider
            parent::excluir_carta_hand('id', 206);
            $existe = true;
            break;
        }
    }
    ...
    if($x > 5) {
     parent::avisar('Não existe espaço para invocar o Dokurorider');
     $this->tarefa('x');
     return false;
    }
     parent::avisar('Carta ritual '.$this->nome.' ativada', 1);
     $this->duelo->invocar($this->dono, $x, 1, 206, 'ritual'); // invocando de forma ritual
     $this->tarefa('x');
     return true;
    }
    else {
     parent::avisar('Dokurorider não foi encontrado na sua mão');
