<?php
class c_254 extends Magica {
/* carta terminada dia 25/05/2017. terminada em 1 dia
 * Esta carta é usada para Ritual Summon (Reshef the Dark Being).
 * Você precisa, também, sacrificar monstros em sua mão ou no seu lado do campo]
 * cujos níveis combinados sejam 8 ou mais.
 */
...
function carta_solicitada($cartaS) {
 ...
 function ritual() { // essa função é executada depois dos sacrificios serem feitos
    ...
    $existe = false;
    for($x = 1; $x < $mao[0]; $x++) {
        if($mao[$x] == 602) { // id da carta Reshef the Dark Being
            parent::excluir_carta_hand('id', 602);
            $existe = true;
            break;
        }
    }
    ...
    if($x > 5) {
     parent::avisar('Não existe espaço para invocar o Chakra');
     $this->tarefa('x');
     return false;
    }
     parent::avisar('Carta ritual '.$this->nome.' ativada', 1);
     $this->duelo->invocar($this->dono, $x, 1, 602, 'Final Ritual of the Ancients'); // invocando de forma ritual
     $this->tarefa('x');
     return true;
    }
    else {
     parent::avisar('Chakra não foi encontrado na sua mão');
