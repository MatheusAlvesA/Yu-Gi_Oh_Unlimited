<?php
class c_292 extends Magica {
/* carta terminada dia 15/06/2017. terminada em 1 dia
 * Esta carta é usada para Ritual Summon (Garma Sword).
 * Você também deve oferecer como Tributo um monstro cujos o níveis totais igual ou superior a 7 do campo ou a sua mão.
 */
...
function carta_solicitada($cartaS) {
 ...
 function ritual() { // essa função é executada depois dos sacrificios serem feitos
    ...
    $existe = false;
    for($x = 1; $x < $mao[0]; $x++) {
        if($mao[$x] == 291) { // id da carta Garma Sword
            parent::excluir_carta_hand('id', 291);
            $existe = true;
            break;
        }
    }
    ...
    if($x > 5) {
     parent::avisar('Não existe espaço para invocar o Garma Sword');
     $this->tarefa('x');
     return false;
    }
     parent::avisar('Carta ritual '.$this->nome.' ativada', 1);
     $this->duelo->invocar($this->dono, $x, 1, 291, 'ritual'); // invocando de forma ritual
     $this->tarefa('x');
     return true;
    }
    else {
     parent::avisar('Garma Sword não foi encontrado na sua mão');
