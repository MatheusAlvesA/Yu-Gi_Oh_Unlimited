...
/* modificado dia 16/03/2017 terminado em um minuto por já ter igual
 * Aumente o ATK e DEF de um monstro demonio equipado com esta carta em 300 pontos.
 */

class c_168 extends Magica {
 ...
 function ativar_efeito() {
  ...
  if(parent::ler_variavel('equipado_em') != 0) {return false;}
   ...
   for($x = 1; $x <= 5; $x++) {
    ...
    if(!$lista) {
        parent::avisar('Não foram encontrados monstros tipo Demonio em no seu campo');
        parent::destruir();
        return false;
    }
    ...
    for($x = 1; $x <= 5; $x++) {
     ...
     if($monstro->set_equip($this->inst)) {
		...
		$infos = $grav->ler(0);
         ...
         parent::avisar('Esse monstro já está equiado');
         parent::destruir();
         return false;
     }
}
