<?php
// terminada 10/03/2017
// terminada em 1 minuto por já existir outras iguais

class c_152 extends Magica {
/* Esta carta é utilizada para o Ritual Summon de (The Masked Beast).
 * Você deve Tributar monstros do seu lado do campo e ou da sua mão até obter um Nível total igual a 8 ou maior.
 */
...
function carta_solicitada($cartaS) {
 ...
 function ritual() { // essa função é executada deposi dos sacrificios serem feitos
    ...
    $id_carta = 722; // id da carta The Masked Beast
    $existe = false;
    for($x = 1; $x < $mao[0]; $x++) {
        if($mao[$x] == $id_carta) {
            parent::excluir_carta_hand('id', $id_carta);
            $existe = true;
            break;
        }
    }
    ...
    if($x > 5) {
     parent::avisar('Não existe espaço para invocar o Magician of The Masked Beast');
     $this->tarefa('x');
     return false;
    }
     parent::avisar('Carta ritual '.$this->nome.' ativada', 1);
     $this->duelo->invocar($this->dono, $x, 1, $id_carta, 'ritual'); // invocando de forma ritual
     $this->tarefa('x');
     return true;
    }
    else {
     parent::avisar('Magician of The Masked Beast não foi encontrado na sua mão');
