<?php
class c_68 extends Magica {
/* Esta carta é utilizada para Ritual Summon (Black Luster Soldier). 
 * Você deve também Tributar monstros cujo total de Níveis se iguale a 8 
 * ou mais do campo ou da sua mão.
 */
...
function carta_solicitada($cartaS) {
 ...
 if($this->ler_variavel('lvs_acumulados') < 8) { // ainda não tem nível acumulado suficiente
  $this->ativar_efeito();
  return true;
 }
 ...
 function ritual() { // essa função é executada deposi dos sacrificios serem feitos
    ...
    $existe = false;
    for($x = 1; $x < $mao[0]; $x++) {
        if($mao[$x] == 69) { // id da carta Black Luster Soldier
            parent::excluir_carta_hand('id', 69);
            $existe = true;
            break;
        }
    }
    ...
    if($x > 5) {
     parent::avisar('Não existe espaço para invocar o Black Luster Soldier');
     $this->tarefa('x');
     return false;
    }
     parent::avisar('Carta ritual '.$this->nome.' ativada', 1);
     $this->duelo->invocar($this->dono, $x, 1, 69, 'ritual'); // invocando de forma ritual
     $this->tarefa('x');
     return true;
    }
    else {
     parent::avisar('Black Luster Soldier não foi encontrado na sua mão');
