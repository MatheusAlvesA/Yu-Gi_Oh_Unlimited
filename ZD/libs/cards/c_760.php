<?php
// terminada 02/03/2017
// terminada em 1 minuto por já existir outras iguais

class c_760 extends Magica {
/* Esta carta é usada para o Ritual Summon de (Crab Turtle).
 * Você também deve oferecer monstros cujo Nível total de Estrelas seja igual a 8
 * ou mais do campo ou da sua mão como um Tributo.
 */
...
function carta_solicitada($cartaS) {
 ...
 function ritual() { // essa função é executada deposi dos sacrificios serem feitos
    ...
    $id_carta = 145; // id da carta Crab Turtle
    $existe = false;
    for($x = 1; $x < $mao[0]; $x++) {
        if($mao[$x] == $id_carta) {
    ...
    if($x > 5) {
     parent::avisar('Não existe espaço para invocar o Crab Turtle');
     $this->tarefa('x');
     return false;
    }
     ...
     parent::avisar('Crab Turtle não foi encontrado na sua mão');
