<?php
class c_123 extends Magica {
/* carta terminada dia 26/09/2016. terminada em 1 dia
 * Esta carta é usada para Ritual Summon (Performance of Sword).
 * Você também deve oferecer monstros cujos total Level em estrelas
 * igual ou superior a 6 do campo ou a sua mão como um tributo.
 */
...
function carta_solicitada($cartaS) {
 ...
 function ritual() { // essa função é executada depois dos sacrificios serem feitos
    ...
    $existe = false;
    for($x = 1; $x < $mao[0]; $x++) {
        if($mao[$x] == 555) { // id da carta Performance of Sword
            parent::excluir_carta_hand('id', 555);
            $existe = true;
            break;
        }
    }
    ...
    if($x > 5) {
     ...
     $this->duelo->invocar($this->dono, $x, 1, 555, 'Commencement Dance'); // invocando de forma ritual
     $this->tarefa('x');
     return true;
    }
    else {
