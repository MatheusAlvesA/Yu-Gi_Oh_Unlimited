 ...
 * Quando essa carta é Normal Summon: Você pode Special Summon 1 monstro de Level 4 (Constellar) da sua mão.
 */
class c_126 extends Monstro_normal {
	...
	function invocar($local, $id, $modo, $dono, $tipo = 'comum', $flags = false) {
		...
		$retorno = parent::invocar($local, $id, $modo, $dono, $tipo, $flags);
                if($tipo == 'comum' && $retorno) $this->_ativar();
                return $retorno;
 	}
 ...
 private function _ativar() {  // Efeito desse monstro
    ...
    for($x = 1; $x < $mao[0]; $x++) {
        $carta->ler_id($mao[$x]);
        if($this->isConstellar($mao[$x]) && $carta->lv == 4) { // id de um monstro constellar
