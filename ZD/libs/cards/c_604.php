<?php
class c_604 extends Magica {
/* carta terminada dia 24/09/2016. terminada em 1 dia
 * Esta carta é usada para o Ritual Summon (Chakra).
 * Você deve também oferecer monstros como um Tributo
 * cujo total de Níveis seja igual a 7 ou maior do seu campo ou da sua mão.
 */
...
function carta_solicitada($cartaS) {
 ...
 function ritual() { // essa função é executada depois dos sacrificios serem feitos
    ...
    $existe = false;
    for($x = 1; $x < $mao[0]; $x++) {
        if($mao[$x] == 114) { // id da carta Chakra
            parent::excluir_carta_hand('id', 114);
            $existe = true;
            break;
        }
    }
    ...
    if($x > 5) {
     ...
     $this->duelo->invocar($this->dono, $x, 1, 114, 'Resurrection of Chakra'); // invocando de forma ritual
     $this->tarefa('x');
     return true;
    }
    else {
