 ...
 * Um monstro LIGHT equipado ganha 400 de ATK e perde 200 de DEF.
 */

class c_235 extends Magica {
 ...
 function ativar_efeito() {
  ...
  if(parent::ler_variavel('equipado_em') != 0) {return false;}
   ...
   for($x = 1; $x <= 5; $x++) {
    ...
    if(!$lista) {
        parent::avisar('Não foram encontrados monstros tipo Light em no seu campo');
        parent::destruir();
        return false;
    }
    ...
    for($x = 1; $x <= 5; $x++) {
     ...
     if($monstro->set_equip($this->inst)) {
		...
		$infos = $grav->ler(0);
	        ...
	        $infos[4] -= 200; // defesa
                if($infos[4] < 0) {
                    parent::manter('def_o', $infos[4]+200);
                    $infos[4] = 0;
                }
                $grav->set_array($infos);
                $grav->gravar();
...
function destruir($motivo = 0) {
    ...
    $monstro->manter('equip', '0');
		...
		$infos = $grav->ler(0);
                $infos[3] -= 400; // ataque
                if(parent::ler_variavel('def_o') != 0) $infos[4] = (int)parent::ler_variavel ('def_o');
	        else $infos[4] += 200; // defesa
                $grav->set_array($infos);
                $grav->gravar();
                unset($grav);
         // efeito desativado
