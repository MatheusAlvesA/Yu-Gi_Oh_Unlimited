<?php
// terminada 19/02/2017
// terminada em 1 minuto por já existir outras iguais

class c_140 extends Magica {
/* Esta carta é usada para Ritual Summon (Dark Master - Zorc).
 * Você deve também oferecer monstros cujo nível totalize 8 ou mais como Tributo do seu campo ou mão.
 */
...
function carta_solicitada($cartaS) {
 ...
 function ritual() { // essa função é executada deposi dos sacrificios serem feitos
    ...
    $id_carta = 177; // id da carta Dark Master - Zorc
    $existe = false;
    for($x = 1; $x < $mao[0]; $x++) {
        if($mao[$x] == $id_carta) {
    ...
    if($x > 5) {
     parent::avisar('Não existe espaço para invocar o Dark Master - Zorc');
     $this->tarefa('x');
     return false;
    }
     ...
     parent::avisar('Dark Master - Zorc não foi encontrado na sua mão');
