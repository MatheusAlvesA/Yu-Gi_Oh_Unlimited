<?php
class c_90 extends Monstro_normal {
    // carta terminada dia 07/09/2016 terminada em 1 dia
/*Quando esta carta é destruída em batalha e enviada ao Cemitério, compre 1 carta.*/
...
function atacado($ataque) {
  ...
  if($resposta['resultado'] == 'v' || $resposta['resultado'] == 'e') { // efeito dessa carta
      $this->_ativar();
  }
  return $resposta;
  }
	 ...
	 if(parent::ler_variavel('atacou_em') == parent::ler_turno()) {parent::avisar('Você não pode atacar mais de uma vez com o mesmo monstro no mesmo turno'); return 0;}
	  ...
	  elseif($resposta['resultado'] == 'v') {
	   ...
	   parent::avisar($this->nome.' atacou '.$alvo->nome.' que foi destruído', 1);
         ...
         if(!file_exists($this->pasta.$this->inst.'.txt')) {$this->_ativar();} // caso tenha sido destruido
         return $resposta;
        }
}

function _ativar() { // unico efeito desse monstro
    $gatilho[0] = 'monstro';
    $gatilho[1] = 'efeito';
    $gatilho[2] = 'self';
	if(parent::checar($gatilho)) {
	 $carta = &parent::checar($gatilho);
	 $resposta = $carta->acionar($gatilho, $this);
	 if($resposta['bloqueado']) {return false;} // foi impedido de ativar
        }
      $this->duelo->puxar_carta($this->dono);
      parent::avisar('Efeito da carta '.$this->nome.' ativado, uma carta foi puxada');
      return true;
}
