...
<html lang="pt">
  <head> <?php include 'head.txt';?>
    ...
    <title>Clãs Yu-Gi-Oh Unlimited</title>
    ...
    <div class="table-responsive col-md-12">
        <table class="table table-striped" cellspacing="0" cellpadding="0">
            <thead>
                <tr>
                    <th>Nome</th>
                    <th class="actions"><b style="float: right;">Pontos</b></th>
                 </tr>
            </thead>
            <tbody>
...
function lista($nome) {
 $t = new Tools();
 if($nome == '') {return '<tr><td colspan="2"><b>Nenhum Clã encontrado</b></td></tr>';}
 if(!$t->verificarstr($nome)) {return '<tr><td colspan="2"><b>Nenhum Clã encontrado</b></td></tr>';}
 unset($t);
 
 $clan = new DB_clan();
 $array = $clan->tabela_clans();

$x = 0;
$y = 0; // controla a linha da matriz
 while(count($array) > $y && $x <= 100) {
   if(str_igual($nome, $array[$y]['nome']) == 1) {$clan->ler($array[$y]['nome']); $retorno = $retorno.'<tr><td><a href="info_clan.php?nome='.$array[$y]['nome'].'"><b>'.$array[$y]['nome'].'</b></a></td><td><b style="float: right;">'.$clan->total_pts().'</b></td></tr>'."\n"; $x++; }
   if(str_igual($nome, $array[$y]['nome']) == 2) {$clan->ler($array[$y]['nome']); $retorno = '<tr><td><a href="info_clan.php?nome='.$array[$y]['nome'].'"><b>'.$array[$y]['nome'].'</b></a></td><td><b style="float: right;">'.$clan->total_pts().'</b></td></tr>'."\n".$retorno; $x++; }
   $y++;
 }
if($retorno == '') {return '<tr><td colspan="2"><b>Nenhum Clã encontrado</b></td></tr>';}
return $retorno;
}
